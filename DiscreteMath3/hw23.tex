...
\begin{proof}[解答]
    ...
    \[
        ...
        \le e^{p_i (e^t - 1)}
    \]
    が成り立つ．ここでは任意の実数 $x$ に対して $1+x \le e^x$ という基本不等式を用いた．
    ...
    あとは $t<0$ をうまく選ぶ．$0<\delta<1$ のとき $\ln(1-\delta) < 0$ なので，
    \[
        t := \ln(1-\delta)
    \]
    とおくと $e^t = 1-\delta$ であり，
    \[
        e^t - 1 - t(1-\delta)
    ...
    \begin{align*}
        ...
        \right)^{\mathbb{E}(X)}.
    \end{align*}
    これはちょうど \eqref{eq:target} の右辺である．したがって，式 \eqref{eq:target} が示された．
\end{proof}
