...
\begin{proof}[解答]
    ...
    \]
    と定める．今回は $\Omega$ が有限集合であるから，事象としてその全ての部分集合を
    考えてよく，
    \[
        \mathcal{F} = 2^\Omega
    \]
    とおくことにする．$2^\Omega$ は $\Omega$ 上の $\sigma$-加法族になっているので，
    $(\Omega,\mathcal{F})$ は測度を入れることができる。
    ...
    $j=i$ なら $1/5$，$j\neq i$ なら $1/2$ とする．このように定めれば，
    3 枚のコインを独立に投げるという仮定と整合する．
    ...
    つぎに，「実際の結果が $A$ であった」という条件のもとでの確率を考える．Homework 9 と同様に，
    \[
        \Pr_A(F) := \Pr(F \mid A) = \frac{\Pr(F\cap A)}{\Pr(A)} \qquad(F\in\mathcal{F})
    \]
    と定めると，$\Pr_A$ は $\mathcal{F}$ 上の確率測度になり，
    $(\Omega,\mathcal{F},\Pr_A)$ は確率空間になる．この確率空間において
    \[
        \Pr(E_2 \mid A) = \Pr_A(E_2)
    \]
    と解釈していることになる．（必要なら，$\Omega$ の代わりに $A$ を標本空間とし，
    $\mathcal{F}_A = \{F\cap A \mid F\in\mathcal{F}\}$ を事象全体とする
    $(A,\mathcal{F}_A,\Pr_A)$ を考えてもよいが，どちらでも本質的には同じである．）
    ...
    \]
    と定める．すると $\Pr_{E_1}$ も $\mathcal{F}$ 上の確率測度になり，
    $(\Omega,\mathcal{F},\Pr_{E_1})$ は確率空間になる．この空間において
    \[
        \Pr(A \mid E_1) = \Pr_{E_1}(A)
    \]
    として理解している．（ここでも，標本空間を $E_1$ に制限した
    $(E_1,\mathcal{F}_{E_1},\Pr_{E_1})$ を考えてもよい．）
